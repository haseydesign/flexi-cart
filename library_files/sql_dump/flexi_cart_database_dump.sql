...
CREATE TABLE `cart_config` (
  ...
  `config_custom_status_1` tinyint(1) NOT NULL,
  `config_custom_status_2` tinyint(1) NOT NULL,
  `config_custom_status_3` tinyint(1) NOT NULL,
  PRIMARY KEY (`config_id`),
  KEY `set_id` (`config_id`) USING BTREE
...
CREATE TABLE `item_stock` (
  ...
  UNIQUE KEY `item_id` (`stock_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `locations` (
  ...
  PRIMARY KEY (`loc_id`),
  UNIQUE KEY `locc_id` (`loc_id`) USING BTREE,
...
CREATE TABLE `order_summary` (
  ...
  UNIQUE KEY `ord_number` (`ord_order_number`) USING BTREE,
...
CREATE TABLE `reward_points_converted` (
  ...
  KEY `rew_convert_reward_fk` (`rew_convert_ord_detail_fk`) USING BTREE,
...
CREATE TABLE `shipping_rates` (
  ...
  UNIQUE KEY `srate_id` (`ship_rate_id`) USING BTREE,
  KEY `srate_ship_fk` (`ship_rate_ship_fk`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `tax` (
  ...
  KEY `tax_country_fk` (`tax_location_fk`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `tax_item_rates` (
  ...
  KEY `tax_item_prd_fk` (`tax_item_item_fk`),
